...
const RegisterForm: React.FC<RegisterFormProps> = ({ onBack }) => {
  const [credentials, setCredentials] = useState<RegisterCredentials>({
    ...
    document_type_id: 1, // Cédula de Ciudadanía por defecto
    info: ''
  });
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    setCredentials(prev => ({
      ...prev,
      [name]: name === 'role_id' || name === 'document_type_id' ? parseInt(value) : value
    }));
  ...
  const isFormValid = () => {
    return credentials.document && 
           ...
           validateDocument(credentials.document, credentials.document_type_id) &&
           Object.values(passwordRequirements).every(req => req) &&
           acceptedTerms;
  };

  const selectedDocumentType = documentTypes.find(dt => dt.id === credentials.document_type_id);

  return (
    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 
                    bg-white/10 backdrop-blur-[10px] p-8 rounded-[20px] text-center text-white 
                    w-[500px] shadow-[0_15px_35px_rgba(0,0,0,0.2)] border border-white/10 
      ...
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-white mb-2 text-shadow-[2px_2px_4px_rgba(0,0,0,0.3)]">
          VetInHouse
        </h1>
        <h2 className="text-xl text-white mb-6">Registro</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="relative">
          <select
            ...
            value={credentials.document_type_id}
            onChange={handleInputChange}
            className="w-full py-3 px-4 bg-white/10 border-none rounded-full text-white text-sm 
          ...
          <div className="absolute right-4 top-1/2 transform -translate-y-1/2 text-[#5FD068] pointer-events-none">
        ...
        {/* Número de Documento */}
        <div className="relative">
          <div className="absolute left-4 top-1/2 transform -translate-y-1/2 text-[#5FD068] text-lg">
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
          ...
          <input
            ...
            className="w-full py-3 px-12 bg-white/10 border-none rounded-full text-white text-sm 
        ...
        <div className="relative">
          <textarea
            ...
            rows={2}
            className="w-full py-3 px-4 bg-white/10 border-none rounded-[15px] text-white text-sm 
        ...
        {credentials.password && (
          <div className="mb-4">
            <div className="h-[3px] bg-white/20 rounded-[5px] mb-2 overflow-hidden">
              <div 
        ...
        {credentials.password && (
          <div className="text-left mb-4 bg-white/10 p-3 rounded-[10px]">
            <p className="mb-2 text-xs">La contraseña debe contener:</p>
        ...
        {credentials.document && !validateDocument(credentials.document, credentials.document_type_id) && (
          <div className="text-left mb-4 bg-red-500/20 p-3 rounded-[10px] border border-red-500/30">
            <p className="text-xs text-red-300">
              ❌ Número de documento inválido para {selectedDocumentType?.name}
            </p>
          </div>
        )}
        ...
        <div className="flex items-start space-x-3 mb-4">
          <input
            ...
            required
          />
          <label htmlFor="acceptTerms" className="text-xs text-white/80 text-left">
        ...
        <button
          ...
          className={`w-full py-3 px-6 border-none rounded-full text-white text-sm font-semibold 
        ...
        {message && (
          <p className={`mt-3 text-xs ${messageType === 'error' ? 'text-[#ff6b6b]' : 'text-[#4CAF50]'}`}>
            {message}
          </p>
        )}
